...
class Sampler:
	def __init__(self, num_training_steps=1000, beta_start=0.0001, beta_end=0.02) -> None:
	...
	def remove_noise(self, input, timestep, predicted_noise):
		assert (input.shape == predicted_noise.shape), "Shapes of noise pattern and input image must match"
		b,c,h,w = input.shape
		device = input.device
...
class SelfAttention(nn.Module):
	def __init__(self, in_channels, num_heads=12, attn_p=0, proj_p=0, fused_attn=True) -> None:
		super().__init__()
...
class MLP(nn.Module):
	def __init__(self, in_channels, mlp_ratio=4, act_layer=nn.GELU, mlp_p=0) -> None:
		super().__init__()
...
class TransformerBlock(nn.Module):
	def __init__(self, in_channels, fused_attention=True, num_heads=4, mlp_ratio=2, proj_p=0, attn_p=0, mlp_p=0, act_layer=nn.GELU, norm_layer=nn.LayerNorm) -> None:
		super().__init__()
...
class Diffusion(nn.Module):
    def __init__(self,
        ...
        self.sinusoid_time_embeddings = SinusodialTimeEmbedding(time_embed_dim=self.time_embed_dim,
...
def sample_plot_image(step_idx,
    ...
    plt.tight_layout()
    for ax, image in zip(axes.ravel(), images_to_vis): #type:ignore
        ax.imshow(image)
        ax.axis("off")
